...
bool Book::compare_argument(std::string argument) {
    std::stringstream ss;
    ss << year << ", " << name << ", " << author;
...
bool Journal::compare_argument(std::string argument) {
    std::stringstream ss;
...
void Database::find(const std::string &key) {
    ...
    int elCount = 0;
    for (int i = 0; i < (int) db.size(); i++) {
        if (db[i]->compare_argument(key)) {
            db[i]->print_info();
            elCount++;
        }
    }
    std::cout << "+" << std::string(4, '-') << "+" << std::string(53, '-') << "+\n";
    std::cout << "| " << "Total: " << elCount << std::string(49, ' ') << "|" << std::endl;
    std::cout << "+" << std::string(58, '-') << "+\n";
}
